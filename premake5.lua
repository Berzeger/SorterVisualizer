...
project "Sorter"
	...
	filter "system:windows"
		postbuildcommands
        	{
            		{ "{COPY} %{prj.name}/dependencies/SDL-2.30.10/lib/x64/SDL2.dll bin/%{cfg.buildcfg}" },
			{ "{COPY} %{prj.name}/dependencies/SFML-2.6.2/bin/sfml-graphics-2.dll bin/%{cfg.buildcfg}" },
			{ "{COPY} %{prj.name}/dependencies/SFML-2.6.2/bin/sfml-system-2.dll bin/%{cfg.buildcfg}" },			
			{ "{COPY} %{prj.name}/dependencies/SFML-2.6.2/bin/sfml-window-2.dll bin/%{cfg.buildcfg}" },
     		}
